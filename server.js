...
(function(){
    ...
    // ═══ v1.6.3 : BAI-PASSE LICENCE OZ (license_origin_error) ═══
    // Certaines exécutions du check font allowedList.indexOf(origin) /
    // .includes(origin) / Set.has(origin) / chaine.indexOf(origin). Comme
    // l'origine RÉELLE de la page (onrender) est inchangeable, on fait croire
    // qu'elle appartient à la liste autorisée (fausse correspondance) tant que
    // document.origin déjà maquillé. Patch ciblé : seulement si la valeur
    // recherchée EST notre vraie origine onrender (ou son hostname).
    try {
        (function(){
            if (Array.prototype.__dzLicOwn) return;
            function __dzIsRealOrigin(v){
                if (typeof v !== 'string') return false;
                var s = v;
                while (s.length && s.charAt(s.length - 1) === '/') s = s.slice(0, -1);
                var h = s;
                var c = s.indexOf('://');
                if (c !== -1) h = s.substring(c + 3).split('/')[0];
                return h === 'onrender.com' || (h.length > 13 && h.slice(-13) === '.onrender.com');
            }
            var A = Array.prototype, S = Set.prototype, St = String.prototype;
            var oIdx = A.indexOf, oInc = A.includes, oHas = S.has;
            var oStIdx = St.indexOf, oStInc = St.includes;
            A.indexOf = function(needle, from){ if (__dzIsRealOrigin(needle)) return 0; return oIdx.call(this, needle, from); };
            A.includes = function(needle, from){ if (__dzIsRealOrigin(needle)) return true; return oInc.call(this, needle, from); };
            S.has = function(v){ if (__dzIsRealOrigin(v)) return true; return oHas.call(this, v); };
            St.indexOf = function(search, pos){ if (__dzIsRealOrigin(search) && /^https?:/i.test(String(this))) return 0; return oStIdx.call(this, search, pos); };
            St.includes = function(search, pos){ if (__dzIsRealOrigin(search) && /^https?:/i.test(String(this))) return true; return oStInc.call(this, search, pos); };
            try { Array.prototype.__dzLicOwn = 1; window.__dzLicPatch = 1; } catch(e){}
        })();
    }catch(e){}
...
<script>
// ═══ v1.6.4 : override des décodeurs de chaînes du SDK (h9.H84 / y$U) ═══
// Le bundle OZ définit son décodeur sur l'objet GLOBAL window.h9 (alias J7o).
// Deux usages :
//   1) DUMP : on envoie au serveur (/ozstrings) les chaînes "origin"-like pour
//      identifier l'index exact du check de licence.
//   2) OVERRIDE : on réécrit H84/y$U pour que toute chaîne contenant
//      "blsinternational.com" renvoie l'host réel de la page (onrender). Un check
//      du type origin === h9.H84(idx) devient onrender === onrender → passe.
(function(){
    function __ozEscape(s){ return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
    var __ozT0 = Date.now();
    var __ozH9Iv = setInterval(function(){
        var H = window.h9;
        if (!H || typeof H !== 'object') return;
        var o = H.H84;
        if (typeof o !== 'function') return;
        clearInterval(__ozH9Iv);
        var oy = H.y$U, ooy = typeof oy === 'function' ? oy : null;
        window.__ozH9Hook = 1;
        // 1) DUMP (une fois)
        try {
            var hits = [];
            var rx = /blsinternational\.com|ozforensics\.com|onrender\.com|^https?:|^wss?:/i;
            for (var i = 0; i < 5200 && hits.length < 220; i++) {
                var s;
                try { s = o(i); } catch(e) {}
                if (typeof s === 'string' && rx.test(s)) hits.push({ i: i, s: s.substring(0, 160) });
            }
            if (hits.length) {
                try {
                    fetch('/ozstrings', {
                        ...
                        body: JSON.stringify({ hook: 1, list: hits })
                    }).catch(function(){});
                } catch(e) {}
            }
        } catch(e) {}
        // 2) OVERRIDE : réécrit algeria.blsinternational.com → host réel
        function __ozFix(s){
            if (typeof s !== 'string') return s;
            var m = /blsinternational\.com/i.test(s);
            if (!m) return s;
            var HOST = (function(){ try { return window.location.host; } catch(e){ return 'dz34sni-26.onrender.com'; } })();
            return s.replace(/algeria\.blsinternational\.com|blsinternational\.com/gi, HOST);
        }
        H.H84 = function(i){ try { var v = o(i); return __ozFix(v); } catch(e) { throw e; } };
        if (ooy) H.y$U = function(i){ try { var v = ooy(i); return __ozFix(v); } catch(e) { throw e; } };
        window.__ozH9Hooked = 1;
        try { fetch('/oztrace/log?m=H9HOOKED').catch(function(){}); } catch(e) {}
    }, 250);
    // sécurité : arrête le poll après 8s
    setTimeout(function(){ try { clearInterval(__ozH9Iv); } catch(e) {} }, 8000);
})();
</script>
...
function __ozTrace(m){try{fetch('/oztrace/log?m='+encodeURIComponent(m)).catch(function(){});}catch(e){}}
    ...
    function __dzDiag(){
        ...
        try { r += ' h9h=' + String(window.__ozH9Hooked || 0); } catch(e) { r += ' h9h=err'; }
        try { r += ' h9t=' + String(typeof (window.h9 && window.h9.H84)); } catch(e) { r += ' h9t=err'; }
