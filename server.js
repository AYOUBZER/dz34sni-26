    ...
    try {
        ...
        const preq = lib.request(u, {
            ...
            for (const [k, v] of Object.entries(pres.headers)) {
                if (/^(connection|keep-alive|transfer-encoding|upgrade|content-encoding)$/i.test(k)) continue;
                if (k === 'content-length' && res.getHeader('content-length')) continue;
            ...
            pres.pipe(res, { end: false });
            pres.on('error', (e) => { doLog(false, 'resp: ' + e.message); res.end(); });
        });
