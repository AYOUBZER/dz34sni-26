...
function buildOzPage(req, opts) {
    const { userId, transactionId, realIp, code } = opts;
...
(function(){
    ...
    try {
        ...
        var __fakeLoc = new Proxy(Object.create(Object.getPrototypeOf(__realLoc)), {
    ...
    try {
        ...
        _ozCfg.on_complete = function(r) {
            ...
            _ozCfg.on_error = function(e) {
                ...
                // v1.6.1 : ajoute l'URL réelle (performance) vs origin spoofer pour
                // savoir QUELLE propriété le SDK lit encore.
                var _dbg = '';
                try {
                    var _nav = performance.getEntriesByType('navigation');
                    var _real = (_nav && _nav[0] && _nav[0].name) || '';
                    var _docURI = '';
                    try{ _docURI = String(document.documentURI); }catch(e){}
                    _dbg = 'spoiledOrigin=' + String(window.location.origin) + '|' + String(document.origin) + ' docDomain=' + String(document.domain) + ' docURI=' + _docURI + ' realNav=' + _real;
                } catch (xdbg) {}
                var _code = '${cd}';
                if (_code) {
                    try {
                        fetch('/result/' + encodeURIComponent(_code), {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ status: 'error', error: msg, transactionId: '${tid}', realIp: '${ip}', timestamp: Date.now(), debugOrigin: _dbg })
                        }).catch(function(){});
                    } catch (xb) {}
                }
            };
